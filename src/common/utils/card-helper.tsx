...
export const applyInstantEffect = (gameState: GameState, dispatch: Dispatch,  card: Card) => {
  ...
  if (effect.effectType === CardEffectType.InstantChips) {
    ...
    dispatch(showSnackbar(`Gained ${formatNumber(bonusChips)} chips!`));
  }
}

export const getCardEffectValue = (gameState: GameState, cardEffect: CardEffectType): number => {
  return gameState.cards.filter(
    (card) => card.effect.effectType === cardEffect,
  ).reduce(
